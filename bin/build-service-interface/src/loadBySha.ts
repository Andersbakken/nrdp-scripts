...
export async function loadBySha(options: Options, build: Build): Promise<LoadResponse> {
    ...
    try {
        response = await load(
            `https://build.dta.netflix.com/nrdp/${options.project}/(commit="${build.value}",buildTags="master")`
        );
    } catch (err) {
        response = await load(
            `https://build.dta.netflix.com/nrdp/${options.project}/(rev="${build.value}",buildTags="master")`
        );
    }
