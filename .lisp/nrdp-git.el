...
(magit-define-popup-action 'magit-push-popup ?V "Submit (no jira resolve)" 'magit-submit-no-resolve)
(magit-define-popup-action 'magit-push-popup ?s "Submit pull-request" 'magit-submit-pull-request)
(magit-define-popup-action 'magit-push-popup ?a "Submit all pull-request" 'magit-submit-all-pull-request)
(magit-define-popup-action 'magit-push-popup ?A "Submit all" 'magit-submit-all)
(magit-define-popup-action 'magit-push-popup ?o "Submit all (no jira resolve)" 'magit-submit-all-noresolve)
...
(defun magit-submit (&optional commit noresolve pr)
  (interactive)
  (let ((prev (getenv "GIT_POST_SUBMIT_FLAGS")))
    (setenv "GIT_POST_SUBMIT_FLAGS" (concat "--no-interactive" (if noresolve "" " --resolve")))
    (let ((args (list "submit" "--no-autodetach")))
      (cond ((null pr))
...
(defun magit-submit-no-resolve (&optional commit)
  (interactive)
  (magit-submit commit t))

(defun magit-submit-pull-request (&optional commit)
  (interactive)
  (magit-submit commit nil t))

(defun magit-submit-all (&optional commit noresolve pr)
  (interactive)
  (let ((prev (getenv "GIT_POST_SUBMIT_FLAGS")))
    (setenv "GIT_POST_SUBMIT_FLAGS" (concat "--no-interactive" (if noresolve "" " --resolve")))
    (let ((args (list "submit" "--no-autodetach" "-a")))
      (cond ((null pr))
            ((stringp pr) (setq args (append args (list "--pull-request" pr))))
            (t (setq args (append args (list "--pull-request" (read-from-minibuffer "Name: "))))))
...
(defun magit-submit-all-pull-request (&optional commit)
  (interactive)
  (magit-submit-all commit nil t))

(defun magit-submit-all-noresolve (&optional commit)
  (interactive)
  (magit-submit-all commit t))
