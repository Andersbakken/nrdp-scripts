...
(defun git-buffer-mouse-clicked (event)
  "Function to translate the mouse clicks in a GIT log buffer to
character events"
  (interactive "e")
...
(defun git-root-dir (&optional directory)
  (interactive)
...
(defun git-run-pre-commit-hook ()
  ...
  (let ((files (git-marked-files-state 'added 'deleted 'modified)))
    (or (not files)
        ...
        (let ((index-file (git-make-temp-file "gitidx")))
          (unwind-protect
...
(defun git-do-commit ()
  ...
  (let ((buffer (current-buffer))
        (index-file (git-make-temp-file "gitidx")))
...
(defun git-add-file ()
  ...
  (interactive)
  (let ((files (git-get-filenames (git-marked-files-state 'unknown 'ignored 'unmerged 'modified))))
...
(defun git-create-active-link (start end prop-list)
  ...
  (git-set-extent-property start end 'mouse-face 'highlight)
  (while prop-list
    (git-set-extent-property start end (caar prop-list) (cdar prop-list))
      ...
      (while (not (eobp))
        ...
        (cond ((looking-at "commit \\([0-9a-g]*\\)")
               (let ((commit-match 1))
                 ...
                 (if (match-beginning commit-match)
                     (git-create-active-link (match-beginning commit-match)
                                             (match-end commit-match)
                                             (list (cons 'git-object-type 'commit)
                                                   (cons 'git-object git-cur-commit))))
                 ))
              ((looking-at "^:[0-9]* [0-9]* \\([0-9a-g]*\\)\\(\\.\\.\\.\\)? \\([0-9a-g]*\\)\\(\\.\\.\\.\\)? *\\([A-Z]\\)\t\\(.*\\)")
               (let ((from-match 1)(to-match 3)(mod-match 5)(file-match 6))
                 (setq git-cur-from (match-string from-match))
                 (setq git-cur-to (match-string to-match))
                 (setq git-cur-file (match-string file-match))
                 (if (and (match-beginning to-match) (match-beginning file-match))
                     (git-create-active-link (match-beginning to-match)
                                             (match-end to-match)
                                             (list (cons 'git-object-type 'file)
                                                   (cons 'git-object-filename git-cur-file)
                                                   (cons 'git-object git-cur-to))))
                 (if (and (match-beginning from-match) (match-beginning file-match))
                     (git-create-active-link (match-beginning from-match)
                                             (match-end from-match)
                                             (list (cons 'git-object-type 'file)
                                                   (cons 'git-object-filename git-cur-file)
                                                   (cons 'git-object git-cur-from))))
                 (if (and (match-beginning mod-match) (match-beginning mod-match))
                     (git-create-active-link (match-beginning mod-match)
                                             (match-end mod-match)
                                             (list (cons 'git-object-type 'diff-against)
                                                   (cons 'git-object-filename git-cur-file)
                                                   (cons 'git-object (concat git-cur-commit "~.." git-cur-commit)))))
                 (if (and (match-beginning file-match) (match-beginning file-match))
                     (git-create-active-link (match-beginning file-match)
...
(defun git-show-file (file named)
  "Show File called name."
  (interactive)
  (let* ((coding-system-for-read git-commits-coding-system)
...
(unless git-status-mode-map
  ...
  (easy-menu-define git-menu git-status-mode-map
	...
	["Diff Against Other" git-diff-file-other t])
      ...
      "--------"
      ["Quit" git-quit-current-buffer t])))
...
(defun git-status (dir)
  ...
  (if (file-directory-p (concat (file-name-as-directory dir) ".git"))
      (let ((buffer (or (and git-reuse-status-buffer (git-find-status-buffer dir))
        ...
        (setq git-default-directory dir)
        (cd dir)
        (git-status-mode)
        (git-refresh-status)
        (goto-char (point-min))
