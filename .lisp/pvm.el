;;; pvm.el --- PVM navigation  -*- lexical-binding: t; -*-
(defun pvm-cd (&optional prefix func)
  (interactive "p")
  (let ((opts nil)
        (args (list "--no-color" "--no-autoupdate" "ls")))
    (cond ((stringp prefix) (setq args (append args (list prefix))))
          ((null prefix))
          ((listp prefix) (setq args (append args (list "toolchain"))))
          (t (setq args (append args (list "-toolchain")))))
    (with-temp-buffer
      (apply #'call-process "pvm" nil t nil args)
      (goto-char (point-min))
      (while (re-search-forward "\\([^ ]+@[0-9]+\\) *-" nil t)
        ;; (message "Got line %s" (buffer-substring-no-properties (point-at-bol) (match-end 1)))
        ;; (message "GOT THING [%s]" (match-string 1))
        (push (match-string 1) opts)
        (forward-char)))
    (if (null opts)
        (error "No pvms found")
      (let ((result (completing-read "PVM: " opts nil t)))
        (when result
          (let ((dir (shell-command-to-string (concat "pvm --no-color --no-autoupdate show --installed --installpath " result))))
            (when dir
              (funcall (or func 'find-file) (if (string-match "\n$" dir) (substring dir 0 -1) dir)))))))))

(provide 'pvm)
