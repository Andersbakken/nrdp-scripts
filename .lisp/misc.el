;;; misc.el --- Miscellaneous utilities  -*- lexical-binding: t; -*-
(require 'delsel)
(require 'nrdp-git)
(require 'lsdev)
(require 'thingatpt)
...
(declare-function git-gutter+-revert-hunks "git-gutter+")
(declare-function ag-regexp "ag")
(declare-function auto-side-windows--get-buffer-side "auto-side-windows")
(declare-function auto-side-windows-toggle-side-window "auto-side-windows")
...
(defun is-ancestor (root child)
  "Check if CHILD is an ancestor of ROOT by going upwards."
...
(defun litter-printf (&optional _begin _end)
  (interactive)
                ...
                (let* ((words (split-string (buffer-substring-no-properties match-start match-end) "[ \t\n]" t))
                  ...
                  (while (>= len 0)
                    (unless (string= (nth len words) "final")
                      (cl-pushnew (nth len words) classes :test #'equal)
                      (setq len -1))
                    (cl-decf len))))))))
      classes)))
  ...
  (save-excursion
    (let ((name (file-name-nondirectory (cond ((bufferp fileorbuffer) (buffer-file-name fileorbuffer))
      ...
      (when (string-match "\\.ui.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".ui" t t name)))))
      (when (string-match "\\.ui$" name)
        (setq candidates (nconc candidates (list (replace-match ".ui.h" t t name)))))
      (when (string-match "_p\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".cpp" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".cc" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".C" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".mm" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".m" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".c" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match ".cpp" t t name)))))
      (when (string-match "_p\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match "_p.cpp" t t name)))))
      (when (string-match "\\.h$" name)
        (setq candidates (nconc candidates (list (replace-match "_p.cpp" t t name)))))
      (when (string-match "\\.hpp$" name)
        (setq candidates (nconc candidates (list (replace-match ".cpp" t t name)))))
      (when (string-match "\\.hxx$" name)
        (setq candidates (nconc candidates (list (replace-match ".cxx" t t name)))))
      (when (string-match "_x11\\.cpp$" name)
        (setq candidates (nconc candidates (list (replace-match ".h" t t name)))))
      ...
      (when (string-match ".cpp$" name)
        (setq candidates (nconc candidates (list (replace-match "_p.h" t t name)))))
      (when (string-match ".cpp$" name)
        (setq candidates (nconc candidates (list (replace-match ".h" t t name)))))
      ...
      (when (string-match ".c$" name)
        (setq candidates (nconc candidates (list (replace-match ".h" t t name)))))
      (when (string-match ".cpp$" name)
        (setq candidates (nconc candidates (list (replace-match ".hpp" t t name)))))
      (when (string-match ".cxx$" name)
        (setq candidates (nconc candidates (list (replace-match ".hxx" t t name)))))
      ...
      (when (and (or (string-match "\\.cpp$" name)
        ...
        (setq candidates (nconc candidates (list (buffer-substring-no-properties (match-beginning 1) (match-end 1))))))

      (goto-char (point-min))
      (when (re-search-forward "\\(//\\|/\\*\\) *SWITCH_FILE: *\"" nil t)
        (let ((start (point)))
          (when (search-forward "\"" (line-end-position) t)
            (cl-pushnew (buffer-substring start (point)) candidates :test #'equal))))
...
(defun point-is-at-eol (pos) (save-excursion (goto-char pos) (= pos (line-end-position))))
(defun point-is-at-bol (pos) (save-excursion (goto-char pos) (= pos (line-beginning-position))))
(defun smart-comment-or-uncomment-region (&optional prefix)
  (interactive "P")
  (let ((start (if (region-active-p) (point) (line-beginning-position)))
        (end (if (region-active-p) (mark) (line-end-position)))
  ...
  (let (containers typedefs)
    ...
    (with-temp-buffer
      ...
      (while (and (< (point) (point-max))
        ...
        (let ((before (buffer-substring-no-properties (line-beginning-position) (1- (point)))))
              ...
              (when (find>)
                ...
                (unless (or (string-match "^[^<]*\\(ptr\\|pointer\\|Ptr\\|Pointer\\)" type) (string= name ""))
                  (if (string-match "^ *typedef +" before)
                      (setq typedefs (nconc typedefs (list name)))
                    (cl-pushnew (concat type (if pointer " *" " ") name) containers :test #'equal)))
                (goto-char start)))))
        ...
        (while (and (< (point) (point-max))
          ...
          (if (match-string 1)
              (cl-pushnew (concat
                          ...
                          containers :test #'equal)
            (cl-pushnew (match-string 0) containers :test #'equal))
            ...
            (t
             (save-excursion
               (let ((containers (or (find-containers)
                 ...
                 (cl-pushnew "" containers :test #'equal)
  ...
  (let ((ins (concat "using namespace " namespace ";")))
    (save-excursion
      ...
      (when (not (search-forward ins nil t))
        (goto-char (point-max))
        (if (cond ((re-search-backward "^ *using namespace" nil t) (goto-char (line-end-position)))
                  ((re-search-backward "^ *# *include" nil t) (goto-char (line-end-position)) (insert "\n"))
                  (t nil))
            (insert "\n" ins))))))

(defun nrdp-compiled-bridge (&optional prefix _srcdir)
  (interactive "p")
    ...
    (misc-find-files directory
                     ...
                     (lambda (file)
                       ...
                       (unless (or (misc-is-compiled file) (not (file-writable-p file)))
                         (cl-incf compiled)
                         (condition-case nil
                             (progn (byte-compile-file file) (load (byte-compile-dest-file file)))
                           (error))))
                     (lambda (file)
...
(defvar mkgibbontest-template
  (concat "/*global nrdp*/\n"
          ...
          "    }\n"
          "    // nrdp.l(\"Got Key: \" + JSON.stringify(key));\n"
          "}\n"
          "function networkResponseHandler(event) {\n"
          "    nrdp.l(\"Got response \" + event.state + \" data: \" + event.size);\n"
          "}\n"
          "function onImageLoaded(event) {\n"
          "    nrdp.l(\"Got image loaded \" + JSON.stringify(event));\n"
          "}\n"
          "function onTimeout() {\n"
          "    nrdp.gibbon.load({ url:\"http://en.wikipedia.org/wiki/Leif_Erikson\",\n"
          "                       headers: {\"X-Gibbon-Cache-Control\":\"key=foo,refresh,no-memory-cache\"}\n"
...
(defun cdgibbontest (&optional _test)
  (interactive)
  (unless mkgibbontest-directory
    (error "You have to set mkgibbontest-directory to something."))
...
(defun misc-store-last-compile (&optional _proc)
...
(defun xclip-sentinel (process _event)
...
(defun misc-save-current-macro-to-dot-emacs (name)
  "Save the current macro as named function in your init file."
...
(defun misc-other-window (_count &optional _all-frames)
  (interactive "p")
...
(defun nrdp-misc-cc-forward-sexp (&optional arg)
  ...
  (let ((conditional)
    ...
    (save-excursion
      (goto-char (line-beginning-position))
      (when (and (search-forward-regexp "^ *# *\\([a-z]*\\)" (line-end-position) t)
            ...
            (if (not (search-forward-regexp "^ *# *\\([a-z]*\\)" nil t))
              ...
              (cond ((eq include t)
                     (when (string= (match-string 1) "include")
                       (setq conditional (line-beginning-position))))
