...
(defun backup-file-log (&optional file)
  ...
  (let ((old default-directory))
    ...
    (call-process "git" nil (current-buffer) nil "log" "--pretty=format:%h%ar" "--" (expand-file-name (concat "~/.backups" file)))
    (cd old))
  (goto-char (point-min))
...
(defun backup-file-redisplay ()
  ...
  (let ((old default-directory) (i 1) (replace nil)
    ...
    (dolist (data backup-file-last-data)
      (insert "Revision #" (format revformat i) " -- " backup-file-last-file " -- " (cdr data) "\n")
